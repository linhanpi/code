...
inline bool spfa(){
    ...
    while(!q.empty()){
        ...
        for(int i=0;i<(int)v[now].size();i++){
            ...
            if(!liu||dist[y]<=dist[now]+fei)continue;
            dist[y]=dist[now]+fei;
...
int dfs(int now,int flow){
    ...
    for(int i=nowhu[now];i<(int)v[now].size();i++){
        int y=v[now][i].dao;
        nowhu[now]=i;
        int fei=v[now][i].fei,liu=v[now][i].liu;
        if(!liu||in[y]||dist[now]+fei!=dist[y])continue;
        int maxn=dfs(y,min(liu,flow-used));
        if(!maxn)dist[y]=inf;
        if(maxn){
            v[now][i].liu-=maxn,v[y][v[now][i].fan].liu+=maxn;
            used+=maxn,minfei+=maxn*fei;
        }
        if(used==flow)break;
    }
    in[now]=0;
...
signed main(){
    ...
    tot=2*n*n;
    st=++tot,ed=++tot;
    jiabian(st,haxi(1,1),k,0);
    jiabian(haxi2(n,n),ed,k,0);
    for(int i=1;i<=n;i++){
        for(int j=1;j<=n;j++){
            int a=read();
            if(i!=n)jiabian(haxi2(i,j),haxi(i+1,j),inf,0);
            if(j!=n)jiabian(haxi2(i,j),haxi(i,j+1),inf,0);
            jiabian(haxi(i,j),haxi2(i,j),1,-a);
            jiabian(haxi(i,j),haxi2(i,j),inf,0);
        }
    }
    Dinic();
    cout<<-minfei<<endl;
