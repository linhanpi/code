 ...
 * @Last Modified time: 2023-03-28 17:00:41
 */
#include<bits/stdc++.h>
...
signed main(){
    ...
    for(int i=1;i<=n;i++){
        int a=read();
        if(s[i]==0)jiabian(st,i,a);
        else jiabian(i,ed,a);
    }
    for(int i=1;i<=m;i++){
        int dou=read(),a=read(),k=read();
        for(int j=1;j<=k;j++){
            int b=read();
            if(dou==0)jiabian(i+n,b,inf);
            else jiabian(b,i+n,inf);
        }
        int dao=read();
        sum+=a;
        if(dou==0)jiabian(st,i+n,a+(dao?g:0));
        if(dou==1)jiabian(i+n,ed,a+(dao?g:0));
    }
    ISAP();
